...
function [AUC LargestAccuracy LineHandle] = AUC_Calculate_ROC_Draw(DecisionValues, Label, ROC_Draw_Flag, LineRGB, LineStyle, CutPoint_Flag)
...
% Neuroscience and Learning, Beijing  University, 2013.
...
if ROC_Draw_Flag
    ...
    LineHandle = plot(FP_Array, TP_Array, LineStyle, 'LineWidth', 3, 'color', LineRGB, 'MarkerSize', 11);
    
    if CutPoint_Flag
        for i = 1:length(OptimalIndex)
            hold on;
            plot(FP_Array(OptimalIndex(i)), TP_Array(OptimalIndex(i)), 'o', 'MarkerSize', 20, 'LineWidth', 3, 'color', LineRGB);
    ...
    xlabel('False postive (1-Specificity)', 'FontSize', 25, 'FontWeight', 'bold');
    ylabel('True positive (Sensitivity)', 'FontSize', 25, 'FontWeight', 'bold');
%     for i = 0.1:0.1:0.9
%         plot([0:0.1:1], ones(1,11) * i, 'k', 'LineWidth', 1, 'LineStyle', ':', 'color', [0 0 0]);
%         plot(ones(1,11) * i, [0:0.1:1], 'k', 'LineWidth', 1, 'LineStyle', ':', 'color', [0 0 0]);
%     end
