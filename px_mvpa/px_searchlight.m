function px_searchlight(Ind,VY, Label,VM, SearchOpt, SummaryFunction, OutputFile)
...
c            = round(DIM(:)/2);
xY.xyz       = M(1:3,:) * [c;1];
[xxx, clique] = spm_ROI(xY, XYZmm_cpy);
...
for i = 1:size(XYZ,2)
  ...
  switch lower(SummaryFunction)
    case 'pearson_correlation'
      SLR(i) = corr(Y(:,1), Y(:,2));
      case 'classification'
          %Data   = reshape(Y, 1, DIM(1)*DIM(2)*DIM(3));
          Data = Y';
          SLR(i) = px_classification(Ind,Data, Label, 'Normalize');
  end
end
