function [handles] = px_barwitherr(errors,varargin)
%**************************************************************************
%   [handles] = px_barwitherr(errors,varargin) 
%   For example, px_barwitherr(se,mean)
%   
...
%                                       vector plots
%   08/05/2012  Pengfei Xu  Added output            
%**************************************************************************
...
if nRows > 1
    for col = 1:nCols
        ...
        handles.errorbar(col) = errorbar(mean(x,1),values(xOrder,col),...
            lowerErrors(xOrder,col), upperErrors(xOrder, col), '.k'); 
    end
else
    x = get(get(handles.bar,'children'),'xdata');
    handles.errorbar = errorbar(mean(x,1),values,errors,'.k');
end
