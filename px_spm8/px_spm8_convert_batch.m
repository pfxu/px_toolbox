...
fid = fopen(fullfile(para.op,'convet_info.txt'),'a+');
...
for i = para.vsub
    ...
    if isempty(runlist) 
        if para.flag == 1;
           ...
           error(['There is no folder under ' fullfile(para.ip,[para.pre num2str(i,'%03.0f')]) '.']);
        end
    end
    n = 0;
    runs = length(runlist);
    for j = 1: runs
        if ~isempty(regexp(runlist{j},para.in,'match'))
            ...
            warning (['The folder with the name',para.in,'is not found in',runlist{j},'please check your data']);
        end
    end
end
fclose(fid);
