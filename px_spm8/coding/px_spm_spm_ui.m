...
% @(#)spm_spm_ui.m	1.15.1.4 Andrew Holmes, Karl Friston 98/01/26
...
for stud  = 1:nStud
	...
	% CBU enter condition nos as matrix option; code changes start
        % here
	if bMCond
	  tmp = spm_input('Enter conditions how?',J,'m', ['Standard SPM|'...
		    'Conditions as matrix'],[1 0],1);  
	  J = J + 1;
	else
	  tmp = 1;
	end
	
	if (~tmp)   % Enter conditions as matrix
		cbunP = size(P,1);  % no of scans already entered
		bMRepl = 0;    % turn off replications index (complex)
		cbumenu = str2mat('Standard SPM select with GUI','Scan names as matrix');
		tmp=spm_input('Enter scan names how?',J,'m', cbumenu, [1 0],1);
		J = J+1;
		if tmp  % select scans via the GUI
		        if bMSubj, nSubj = spm_input([sStudP,'# of subjects ?'],J); J=J+1; end
			for subj = 1:nSubj
				sSubjP = [];
				if bMSubj 
					sSubjP = ['Subj.',int2str(subj),': ']; 
				end	
				t_str = [sStudP,sSubjP,'Select scans ',...
					 'all conditions'];
				tP     = spm_get(Inf,'img',t_str);
				P      = str2mat(P,tP);
				iSubj = [iSubj, subj*ones(1,size(tP, 1))];
			end  % for subj
		else  % enter scans via matrix
			tP = spm_input([sStudP 'Scan names'], J);
			J = J + 1;				
		        if bMSubj, 
				cbusubjs = spm_input([sStudP 'Subject nos for scans'],J);
				while prod(size(cbusubjs))~=size(tP, 1);
					fprintf('%d subject numbers when %d expected\n',...
						prod(size(cbusubjs)),...
						size(tP, 1));
					cbusubjs = spm_input([sStudP 'Subjects'],J);
				end % while
				J = J + 1;
				nSubj = max(cbusubjs);
				iSubj = [iSubj cbusubjs(:)'];
			else
				iSubj = [iSubj ones(1, size(tP, 1))];
			end
			P = str2mat(P, tP);
		end  % GUI or matrix	
		% no of scans entered      
		...
		cbuconds = spm_input([sStudP 'Conditions'],J);
		while prod(size(cbuconds))~=tPno;
			fprintf('%d Condition numbers when %d expected\n',...
				prod(size(cbuconds)),...
				tPno);
			cbuconds = spm_input([sStudP 'Condition nos for scans'],J);
		end % while  
		J = J + 1;      
		iCond = [iCond cbuconds(:)'];
		iStud = [iStud stud * ones(1, tPno)];	
	else
