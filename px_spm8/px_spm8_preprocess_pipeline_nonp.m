...
% subject vector
if ~isfield(para,'vsub');
    error('None defined para.vsub.');
end
...
% patermeters of each mode
fprintf('\n%s\n',repmat('=',10));
fprintf('Modes include:\n');
for nf = 1:length(mode)
    ...
    switch(flag)
        ...
        case 'slicetiming'
            ...
            fprintf('%d. slicetiming\n', nf);
        case 'realign'
            fprintf('%d. realign\n', nf);
        case 'coregister_est'
            if ~isfield(para,'core_fwhm');para.core_fwhm = [7 7];end
            core_opt = 0;
            fprintf('%d. coregister_est\n', nf);
        case 'coregister_estwrite'
            if ~isfield(para,'core_fwhm');para.core_fwhm = [7 7];end
            core_opt = 1;
            fprintf('%d. coregister_estwrite\n', nf);
        case 'lesion'
            tbx_cfg_clinical;
            fprintf('%d. lesion segmentation\n', nf);
        case 'segment'
...
for vs = vsub
    ...
    if para.flag == 0;
        ...
        sd = fullfile(df,sn);
        cd(sd);
        fprintf('Starting: Changing directory to: %s\n', sd);
    else
        sd = subj_list{vs};
        cd(sd);
        fprintf('Starting: Changing directory to: %s\n', subj_list{vs});
...
if ~isempty(cell2mat(strfind(lower(mode),'realign')));
    fprintf('Calculate head movement:\n');
