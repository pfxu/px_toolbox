function [para] = px_spm8_preprocess_segment(fdp,para)
%FORMAT [para] = px_spm8_preprocess_segment(fdp,para)
% Usage Call the spm8 for segmentation. 
%   Input 
%     fdp.scan - full path of the data files. Cell format for input, with one 
...
% batch
matlabbatch{1}.spm.spatial.preproc.data = fdp.scan;
...
save (fullfile(para.op,'prep_segment'), 'matlabbatch');
% run
spm_jobman('initcfg');
spm('defaults', 'FMRI');
