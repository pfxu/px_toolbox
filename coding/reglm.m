...
%   数X 是自变量观测值矩阵，它是n 行 p 列的矩阵. y 是因变量观测值向量，它是n 行1 列的列向量. 
% 
%   stats = reglm(y,X)，还返回一个包括了回归分析的所有诊断统计量的结构体变量stats. 
% 
%   stats = reglm(y,X,model)，用可选的model 参数来控制回归模型的类型. model 是一个字符串， 
%   其可用的字符串如下 
...
% 
%   stats = reglm(y,X,model,varnames)，用可选的varnames 参数指定变量标签. varnames  
%   可以是字符矩阵或字符串元胞数组，它的每行的字符或每个元胞的字符串是一个变量的标签，它的行 
%   数或元胞数应与X 的列数相同. 默认情况下，用X1,X2,…作为变量标签. 
% 
%   例: 
...
if nargin < 4 || isempty(varnames) 
    ...
    if size(varname1,1) ~= p 
        error('变量标签数与X 的列数不一致'); 
    else 
        varnames = makevarnames(varname1,model);    % 指定的变量标签 
    end 
end 
 
ST = regstats(y,X,model);    % 调用regstats 函数进行线性回归分析，返回结构体变量ST 
f = ST.fstat;    % F 检验相关结果 
t = ST.tstat;    % t 检验相关结果 
...
fprintf('-------------------------------参数估计-------------------------------'); 
fprintf('\n'); 
fprintf('%8s%18s%15s%15s%12s','变量','估计值','标准误','t 值','p 值'); 
fprintf('\n'); 
