function px_gretna_voxel_based_degree_pipeuse(Data_List, Output_path, Data_mask, R_thr, Dis, name)

%==========================================================================
% This function is used to calculate nodal degree for each voxel in the
% brain. (for GUI pipeline)
%
% Syntax: function px_gretna_voxel_based_degree(Data_List, Output_path, File_filter, Data_mask)
%
% Inputs:
%   Data_List:
...
% reslice them.
if any(Vmask.dim ~= Vin{1}.dim) | any(Vmask.mat ~= Vin{1}.mat)
    fn_original          = Vmask.fname;
    [pathstr, namestr, ext] = fileparts(fn_original);    
    fn_resliced          = fullfile(pathstr,['r' namestr ext]);
    px_spm8_reslice(fn_original,fn_resliced,'',0,Vin{1});
    Vmask = spm_vol(fn_resliced);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
